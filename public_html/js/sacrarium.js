...
function rhan(){
    if(cult.innocents.current >= gods.rhan.cost){
        ...
        eventBox( "Rhan-Tegoth", "She is pleased at the offerings the Priest prepares, but She is ever hungry.(-8 Innocents, Madness +44, Terror +88, Radiance +8, Sacrifice unlocked in TerrorCrafts, Rhan in Sacrarium -Rhan will feed if not appeased, preferring the Insane over all others.)" );
...
function dagon(){
    if(vault.ichor.current>= gods.dagon.cost && stats.radiance.current >= gods.dagon.radCost){
        ...
        eventBox('Dagon', 'Seated on a Thone of coral, Dagon silently grants his Blessing. West can now breathe underwater. (Doubles Reef and Deep Trade effectiveness, Access to Ocean Depths.)');
