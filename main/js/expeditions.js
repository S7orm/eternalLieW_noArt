...
function crypt(){
    if(vault.gold.current > world.crypt.cost && madMin(44)){
        ...
        if(world.crypt.purchased === false){
            ...
            eventBox( 'Pnakotic Manuscripts', 'A unique find. Translation (West tab) will improve Studying and Mental Fortitude. More can be Forged in FleshCrafts.');
...
function tower(){
    if(vault.gold.current >= world.tower.cost && madMin(88)){
        ...
        if(world.tower.purchased === false){
            ...
            eventBox('Shining Trapezohedron', "Its facets reveal the infinite Realms beyond. West catches a glimpse of what could be Unknown Kadath where the gods dwell, so they say. West's dreams are now filled with the search for that fabled locale. (Dream Expeditions unlocked. Trapezohedron placed in Sacrarium.)");
...
function estate(){//unlocks reef bc innsmouth
    if(vault.gold.current >= world.estate.cost && madMin(888)){
        ...
        if(world.estate.purchased === false){
            ...
            eventBox( 'Innsmouth Estate', "Mouldy and delapidated, West's childhood home brings back memories of Family secrets under the earth and what lies beyond the Reef. (Breeding Pits in FleshCrafts, Reef Expedition)'");
...
function ant(){
    if(vault.gold.current >= world.ant.cost && cult.sentinels.current >=44 && madMin(444444)){
        if(world.ant.purchased === false){
            ...
            eventBox("Antarctica", "Gonna be a whole new reset mechanic here... someday...");
