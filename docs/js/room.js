...
function altarInfo(){
    document.getElementById('altarRoomInfo').innerHTML=
            "<p>The Altar Room activates with each toll of the bell. Drag and drop Cultists to fill the room. Doubletap to remove. Hover on sidebar elements for details.</p>";
}
// Parameters
let rows = 4;
let columns = 4;
...
function placePegInHole(pegType, holeId, row, col, isDefault = false) {
    ...
    span.textContent = pegType.charAt(0).toUpperCase();
    span.classList.add('pegInHole');
...
function pegPoolInit(){
    ...
    pegTypes.forEach(name => {
        ...
        span.className="pegLetter";
        const label = document.createElement("div");
        label.className = "peg-label";
        label.textContent = name.charAt(0).toUpperCase() + name.slice(1);
        pegDiv.append(span, label);
...
function startDrag(event, pegType, holeId) { // initiates dragging of a peg
    ...
    const span = originalPeg.querySelector('span'); 
    const dragCopy = span.cloneNode(true); 
    ...
    const rect = span.getBoundingClientRect(); // get original image position
