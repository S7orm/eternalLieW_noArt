...
function rhan(){
    if(cult.insane.current >= gods.rhan.cost && madMin(168) && cult.priests.current >0){
        ...
        eventBox("Rhan-Tegoth", "She is pleased at the offerings the Priest prepares, but She is ever hungry.(-8 Insane, Madness +168, Terror +168, Radiance +8, Sacrifice unlocked in TerrorCrafts, Rhan in Sacrarium -Rhan will feed if not appeased, preferring the Insane over all others.)" );
...
function nyar(){
    if(madMin(248)){
        ...
        shakeBody();
        eventBox("Destiny", "Shall we dance?");
...
function dagon(){
    if(vault.ichor.current>= gods.dagon.cost && stats.radiance.current >= gods.dagon.radCost){
        ...
        eventBox('Dagon', 'Seated on a Thone of coral, Dagon silently grants his Blessing. West gains hidden gills, webbing, and immunity to the crushing Depths. (Doubles Reef and Deep Trade effectiveness, Ocean Depths unlocked.)');
...
function yig(){
    if(stats.radiance.current>=8 && madMin(444) && stats.health.current>=444){
        ...
        eventBox('Yig', "Enduring the agony of the Venom changes West, but the god is pleased. (Health capacity doubled, Snake Handling in TyogCrafts");
       }
}
...
function shub(){//Madness Minimum: 484', 'Offering Required: 88 Innocents, 484 Radiance
    if(cult.innocents.current >= 84 && cult.faithful.current >= 48 && stats.radiance.current >= 484 && madMin(484)){
        ...
        flashFade('shubWrap');
        eventBox('Shub Niggurath', "Iä! Shub Niggurath! Black Goat of the Woods and Mother of 1000! T'yog falls prone at the sight of the Goat and must be prodded to finish the ritual. (Black Goat in Sacrarium, Breeding Pits output quadrupled.)");
...
function cthulhu(){
    if(stats.madness.current >= 848){
        ...
        eventBox( 'Chtulhu', "A terrible cacophany of sensations registers to West's warped senses as the great being's laughter. West's Essence crystalizes across countless eons as timeless beings take notice. (Immortality gained -view in Sacrarium)");
