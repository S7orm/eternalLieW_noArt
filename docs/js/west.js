...
function preach() {
    if (stats.charm.current >=  actions.preach.cost) {
        if(cult.faithful.unlocked === false){
            ...
            eventBox("The Cult", "One alone cannot accomplish Greatness. West must find and use those foolish enough to walk into darkness.");
            setTimeout(() => {
...
function damn(){
     if(stats.vision.current >= actionUpgrades.study.damn.cost){
        ...
        eventBox("Liber Damnatus", "West begins to see the connections between conflicting thoughts. (translating Tomes increases West's Study action and capacity for Madness. Tomes can be manufactured from fragments in FleshCrafts. (TomeCrafts unlocked, +1 Tome in Vault)");
...
function pnak(){//unlocked by crypt
    if(stats.vision.current >= actionUpgrades.study.pnak.cost){
        ...
        eventBox( "Pnakotic Manuscripts", "The 8th fragment describes a dark and bloody ritual, but it is the address scribbled in the margin that catches West's eye. (Wax Museum expedition unlocked, +1 Tome in Vault)");
...
function kult(){//unlocked by tome
    if(stats.vision.current >=actionUpgrades.study.kult.cost ){
        ...
        eventBox( "T'yog", "Ciphered in the text is a clue to the final resting place of T'yog, High Priest of Shub-Niggurath and the key to his release. (Wax Museum expedition updated, Scroll of T'yog in Sacrarium, +1 Tome in Vault)");
...
function vermin(){ //unlocked by tome
    if(stats.vision.current >= actionUpgrades.study.vermin.cost){
        ...
        eventBox("Brown Jenkin", "Summoned through a dark ritual encoded in the text, the man faced rat offers to show West passages to the deepest layer of the Dreaming. Its strangly human hands wring in anticipation. (Brown Jenkin in Sacrarium, Throne of Azathoth dream expedition available)");
