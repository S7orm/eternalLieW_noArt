...
function preach() {
    if (stats.charm.current >=  actions.preach.cost) {
        if(cult.faithful.unlocked === false){
            ...
            eventBox("The Cult", "One alone cannot accomplish Greatness. West must find and use those foolish enough to follow him into darkness.");
            setTimeout(() => {
...
function kult(){//unlocked by tome
    if(stats.vision.current >=actionUpgrades.study.kult.cost ){
        ...
        eventBox( "T'yog", "Ciphered in the text is a clue to the final resting place of T'yog, High Priest of Shub-Niggurath and the key to his release. (Wax Museum, Stone Passage Expeditions updated, Scroll of T'yog in Sacrarium, +1 Tome in Vault)");
