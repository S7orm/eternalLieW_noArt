...
function crypt(){
    if(vault.gold.current > world.crypt.cost && madMin(44)){
        ...
        if(world.crypt.stage===1){
            ...
            eventBox('Liber Damnatus', "Even with Sentinels to assist, it seemed like a futile endeavor to rummage through the bones of the dead. Ia!  In the tomb of a despised ancestor, still clutched in the corpse's final embrace, this Tome glows with power. (Translate Tomes in West tab.)");
            ...
            eventBox('Ancestral Estates', "West's ancestors took their secrets with them when they died. Buried in layers of bone dust, inlaid case of brittle deeds looks authentic. (Ancestral Estate expedition unlocked, Sentinels find Flesh, loose pages acquired)");
            world.crypt.stage=3;
...
function antiquarian(){
    if(vault.gold.current >= world.antiquarian.cost && madMin(88)){
        ...
        if(world.antiquarian.stage===1){
            ...
            eventBox("Marceline ", "The subject is a woman of breathtaking beauty, whose hair waves in an unearthly breeze. The longer West looks, the more the landscape seems to show glimpses of unknown worlds. (Marceline painting in Sacrarium.)");
            ...
            eventBox("Ia! The Viol of Erich Zahn!", "Lost these many years, Its strings still hum with emotional resonance. (Viol placed in Sacrarium)");
...
function tower(){
    if(world.tower.stage===1){//syringe for early murder
        if(vault.gold.current >= world.tower.cost && madMin(164)){
            ...
            eventBox("Little Gold Syringe", "Inside a locked case is a little gold syringe. It appears harmless.(TerrorCrafts, Gold Syringe in Sacrarium");
            towerUpdater(false);
        }
        ...
        if(vault.gold.current >= world.tower.cost && madMin(248)){
            ...
            eventBox("Notes on Essential Salts", "Hidden in a false wall, these crumbling notes detail how to force secrets from the dust of the dead. (Séance in FleshCrafts)");
            world.tower.stage=3;
        ...
        if(vault.gold.current >= world.tower.cost && madMin(484)){
            ...
            eventBox("Pineal Resonator", "This mysterous blending of machine and Madness is a triumph of engineering.(IchorCrafts, Pineal Resonator in Sacrarium)");
            world.tower.stage=4;
        ...
        if(vault.gold.current >= world.tower.cost && madMin(848)){
            ...
            eventBox('Shining Trapezohedron', "While West is transfixed by the vistas shown in the crystal's facets, another, hungrier presence stares back. (Sublimate unlocked in West tab, Trapezohedron placed in Sacrarium.)");
...
function estate(){
    if(world.estate.stage===1){
        ...
        if(vault.gold.current >= world.estate.cost && madMin(world.estate.madMin)){
            ...
            eventBox('Buried Secrets', "Beneath the facade of genteel society lies a monstrous creation. (Breeding Pits in FleshCrafts)'");
        ...
        if(vault.gold.current >= world.estate.cost && madMin(world.estate.madMin)){
            ...
            eventBox( 'Innsmouth Harbor', "An ornate metal anchor hidden in a false wall gives West a rare opportunity. (Coral Reef expedition unlocked)");
        ...
        if(cult.innocents.current>=8 && vault.gold.current >= world.estate.cost && madMin(world.estate.madMin)){
            ...
            eventBox("Mi-Go", "The winged crustaceans from Yuggoth are friendlier than they seem. They offer West one of their brain transport cylinders for study in exchange for only a few Innocents. (Brain Jars unlocked in IchorCrafts)");
...
function wax(){
    if(vault.gold.current > world.wax.cost && madMin(world.wax.madMin)){
        ...
        if(world.wax.stage===1){
            ...
            eventBox("Rhan-Tegoth!", "She was hidden there, locked away in the basement, waiting as she had waited Eons in the frozen North. The Harbinger of the Gods, Rhan is the Vessel of Transmutation. (Rhan waits in the Sacrarium)");
...
function desert(){
    if(world.desert.stage===1){
        if(vault.gold.current >= world.desert.cost && madMin(248)){
            ...
            eventBox('Hidden Passage', 'Having breached the ancient wall, West gazes into the depths and hears a faint piping. There is a flash like lightning and then laughter. (Nyarlathotep waits in the Sacrarium)'); 
        ...
        if(cult.sentinels.current>=8 && vault.gold.current >= world.desert.cost && madMin(248)){
            ...
            eventBox('Irem?', 'Opening trade with the ancient Crocodilian civilzation was perilous, but West is sure they have much to offer. (Crocodilian Bargains)');
...
function tsath(){
    if(world.tsath.stage===1){
        if (vault.gold.current >= world.tsath.cost && cult.sentinels.current>=16 && madMin(world.tsath.madMin)) {
            ...
            tsathUpdater();
            eventBox("Tsath", "With a Sentinel honor guard, T'yog greets the K'n-yan in their ancient tongue; both facts impressing the strange cavern dwellers. In the days after, West learns much from the decadent people. (Rite of Spring?, The Winnowing?)"); 
        }
        ...
        if (vault.gold.current >= world.tsath.cost && cult.sentinels.current>=8 && madMin(world.tsath.madMin) && vault.tome.current>=1) {
            ...
            world.tsath.madMin=484;
            tsathUpdater();
            eventBox( "Scribes", "Pleased at the gift, the K'n-yan leaders show West their Grand Library and the Scribes who filled it. (Scribes unlocked in TomeCrafts)");
        }
        ...
        if (vault.gold.current >= world.tsath.cost && cult.sentinels.current>=4 && madMin(world.tsath.madMin) && stats.vision.current>=8448) {
            ...
            world.tsath.madMin=484;
            tsathUpdater();
            eventBox("Telepathic Communion", "The K'n-yan scholars drain West of his Visions of the upper Earth, but teach West a rudimentary telepathy. (Krueger? in LoveCrafts, Apprentices unlocked in TomeCrafts)");
        }
        ...
        if (vault.gold.current >= world.tsath.cost && madMin(world.tsath.madMin) && vault.ichor.current>=8) {
            ...
            tsathUpdater();
            eventBox("Tulu Altar", "Composed of an alloy brought to Earth by Great Tulu himself, the Altar hums with invisble energies. (Tulu Altar in altar room -this relic which is dangerous to use.)");
        }
...
function yoth(){
     if (vault.gold.current >= world.yoth.cost && cult.sentinels.current>=8 && madMin(484)) {
        ...
        eventBox("Yoth", "Beneath the heart of dead Yoth lie the looted vaults of Zin. At T'yog's reptilian hiss, a shrine to Yig, the prehuman snake diety, is revealed. West is sure that with more funding, the entrance to lightless N'kai would be found. (N'kai expedition unlocked, Yig in Sacrarium)"); 
...
function nkai(){
     if (vault.gold.current >= world.nkai.cost && cult.sentinels.current>=8 && madMin(848)) {
        ...
        eventBox("N'kai","West fearfully approaches lightless N'kai and communes with the slippery things that dwell there. In an underground lake sits massive Tsathoggua, mouth still agape waiting lazily for the sacrifice. (Tsathoggua in Sacrarium)");
...
function reef(){
    if(world.reef.stage===1){
        if(vault.flesh.current >= world.reef.cost && madMin(484)){
            ...
            eventBox('Coral Reef', 'Using the Emerald Soapstone, West summons the Deep Ones. They offer trade and passage to their master Dagon. (Dagon unlocked in sacrarium, Deep Trade in FleshCrafts)');
            world.reef.stage=2;
...
function depths(){
    if(cult.hybrids.current >= world.depths.cost && relics.dagonsBlessing.unlocked === true && madMin(848)){
        ...
        if(world.depths.stage===1){
            ...
            eventBox("R'lyeh", "Despite the predations of sharks and other, less wholesome, creatures of the depths, West manages to find the sunken city. Swimming through its Cyclopean ruins, his essence is warped enough to catch the notice of the sleeper. (Cthulhu unlocked in Sacrarium)");
            world.depths.stage++;
        }else{
            loosePages(8888);
        }
...
function pillar(){
    if(stats.radiance.current>=dreamEx.pillar.cost && stats.vision.current >= dreamEx.pillar.visionCost){
        ...
        eventBox('Lighthouse', 'In a nearby cavern West finds the bearded priests Nasht and Kaman-Thah. They warn him not to seek Unknown Kadath, where the Gods dwell, but agree to keep the Lighthouse burning in exchange for a bit of Radiance.(Dream Options Unlocked in West Tab)');
...
function zoog(){
    if(stats.radiance.current>=dreamEx.zoog.cost && stats.vision.current >= dreamEx.zoog.visionCost){
        ...
        eventBox("Zoog Village", "Speaking in their fluttering tongue, the Zoog know not where to find Kadath, where the Gods dwell. They urge West on to Ulthar and gift him a bottle of Moon-Wine. (Ulthar expedition, 4 x Drinking effectiveness)");
    }
}
...
function ulthar(){
    if(stats.radiance.current>=dreamEx.ulthar.cost && stats.vision.current >= dreamEx.ulthar.visionCost){
        ...
        eventBox( 'Cats of Ulthar', 'A friendly Abyssinian adopts  West as he passes through the quiet city streets. Its Chaming presence proves very calming. (Bast is lounging in the Sacrarium)');
...
function plateau(){
    if(stats.radiance.current>=dreamEx.plateau.cost && stats.vision.current >= dreamEx.plateau.visionCost  && madMin(248)){
        ...
        eventBox('Men of Leng', 'Dangerous looking men invite West to a feast. As he dines, they share stories of secrets buried in the desert. West is so invigorated by the repast he asks for the recipe. They happily oblige and he awakens with a Tome in hands. (Cultes des Goules in West tab, Desert Expedition Unlocked)');
    }
}
...
function zar(){
    if(stats.radiance.current>=dreamEx.zar.cost && stats.vision.current >= dreamEx.zar.visionCost){
        ...
        flashFade('zarWrap');
        eventBox("Land of Zar", "The dreams of dead poets gift West stirring words to sway the Faithful.. (Preach effectiveness increased. Love from Faithful increased)");
...
function xura(){
    if(stats.radiance.current>=dreamEx.xura.cost && stats.vision.current >= dreamEx.xura.visionCost){
        ...
        flashFade('xuraWrap');
        eventBox("Xura", "In the land of pleasures unattained, they smoke a strange oil to forget the pains of life. (Smoke effectiveness greatly increased.)");
...
function sona(){
    if(stats.radiance.current>=dreamEx.sona.cost && stats.vision.current >= dreamEx.sona.visionCost){
        ...
        flashFade('sonaWrap');
        eventBox('Sona-Nyl', 'The Land of Fancy captivates and enriches the soul. (Time continues to pass. West gains idle Vision, Charm, and Radiance until this window is closed and West leaves Sona-Nyl behind forever.)');
...
function dylath(){//88 rad unlock black ship
    if(stats.radiance.current>=dreamEx.dylath.cost && stats.vision.current >= dreamEx.dylath.visionCost  && madMin(888)){
        ...
        eventBox('Dylath-Leen', 'In the docks of Dylath-Leen, the Black Ship awaits. The filthy turbans worn by the sailors do little to hide something even more unclean underneath. They offer West passage to regions untouched by men.(Black Ship dream option in West Tab -very expensive.)');
    }
}
function kla(){
...
function basalt(){
    if(stats.radiance.current>=dreamEx.basalt.cost && stats.vision.current >= dreamEx.basalt.visionCost  && madMin(555)){
        //choice of ship
        eventBox("A Choice", "Black Ship option costs 88 extra Radiance");
...
function basaltWhite(){
    ...
    closeEventBox();
    setTimeout(() => {
        eventBox('Crashed', 'A cruel joke, to think that the Gods could be so easily attained. Passing through the Pillars, the ship is caught in a vast whirlpool. Tumbling down, the White Ship is torn asunder. West can awaken in a cold sweat having lost the White Ship or face the Nameless Mist.');
        let parent = document.getElementById('eventBox');
...
function basaltBlack(){
        ...
        numberChange('stats', 'vision', -dreamEx.basalt.visionCost, 'blue', 'red');
            eventBox('Ascent', "Passing near the yawning abyss gives West pause, but the Black Ship lurches into the sky, oars rowing the empty air wildly. The masters of the ship laugh cruelly at his discomfort and point to a map of the heavens hanging in the navigator's room. (Several new Expeditions available.)");
...
function kadath(){
    if(stats.radiance.current>=dreamEx.kadath.cost && stats.vision.current >= dreamEx.kadath.visionCost){
        ...
        eventBox("Kadath", "Congrats! Here are all the gods. Sorry, not implemented yet");
    }
}
...
function throne(){
    if(stats.radiance.current>=dreamEx.throne.cost && stats.vision.current >= dreamEx.throne.visionCost){
        ...
        eventBox("Sign the Book.",  "End this charade. (Book of Azathoth in West tab)");
