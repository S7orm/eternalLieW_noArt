...
function afk(){
    ...
    if(tics >=8){
        ...
        if(timeAfk>=4){
            eventBox("West Returns", "West was idle for " + timeAfk + " minutes but the Cult remained active and the Gods hunger. (West does not act but is still affected by permanent Madness and Relics.)");
        }
            //end updates
...
function offlineProgress(){
    ...
    eventBox("West Returns", "After West left, the Priests grew indolent, the Sacrarium darkened and the Gods slept once more. Only the cult remains active. (Offline time = " + Math.floor(timePassed/60000) + " minutes ~ 4x last session up to 88 min.)");
}